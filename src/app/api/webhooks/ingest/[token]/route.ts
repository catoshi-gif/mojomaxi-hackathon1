// filepath: src/app/api/webhooks/ingest/[token]/route.ts
...
export async function POST(req: NextRequest, ctx: any) {
  const token = String((ctx?.params?.token ?? ctx?.params?.id) || "").trim();
  if (!token) {
    return NextResponse.json({ ok: false, error: "missing_token" }, { status: 400 });
  }
  return forwardPOST(req, token, "legacy-webhooks-ingest");
}
